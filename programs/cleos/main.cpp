...
Subcommands:
  ...
  transfer                    Transfer BEOS from account to account
...
void get_jurisdictions() {
   if( !jurisdictions.empty() ){
      ...
      if(jj_array.size() >= eosio::chain::jurisdiction_manager::limit_256){
         throw fc::exception(fc::log_messages{FC_LOG_MESSAGE(error, eosio::chain::jurisdiction_manager::too_many_jurisdictions_exception )});
      }
      for( auto j : jj_array) {
   ...
   create_account_subcommand(CLI::App* actionRoot, bool s) : simple(s) {
      ...
      if (!simple) {
        createAccount->add_option("--stake-net", stake_net,
                                  (localized("The amount of BEOS delegated for net bandwidth")));
        createAccount->add_option("--stake-cpu", stake_cpu,
                                  (localized("The amount of BEOS delegated for CPU bandwidth")));
        ...
        createAccount->add_option("--buy-ram", buy_ram_eos,
                                  (localized("The amount of RAM bytes to purchase for the new account in BEOS")));
        createAccount->add_flag("--transfer", transfer,
                                (localized("Transfer voting power and right to unstake BEOS to receiver")));
      }
...
struct delegate_bandwidth_subcommand {
   ...
   delegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      delegate_bandwidth->add_option("stake_net_quantity", stake_net_amount, localized("The amount of BEOS to stake for network bandwidth"))->required();
      delegate_bandwidth->add_option("stake_cpu_quantity", stake_cpu_amount, localized("The amount of BEOS to stake for CPU bandwidth"))->required();
      delegate_bandwidth->add_option("--buyram", buy_ram_amount, localized("The amount of BEOS to buyram"));
      delegate_bandwidth->add_option("--buy-ram-bytes", buy_ram_bytes, localized("The amount of RAM to buy in number of bytes"));
      delegate_bandwidth->add_flag("--transfer", transfer, localized("Transfer voting power and right to unstake BEOS to receiver"));
...
struct undelegate_bandwidth_subcommand {
   ...
   undelegate_bandwidth_subcommand(CLI::App* actionRoot) {
      ...
      undelegate_bandwidth->add_option("unstake_net_quantity", unstake_net_amount, localized("The amount of BEOS to undelegate for network bandwidth"))->required();
      undelegate_bandwidth->add_option("unstake_cpu_quantity", unstake_cpu_amount, localized("The amount of BEOS to undelegate for CPU bandwidth"))->required();
...
struct bidname_subcommand {
   ...
   bidname_subcommand(CLI::App *actionRoot) {
      ...
      bidname->add_option("bid", bid_amount, localized("The amount of BEOS to bid"))->required();
...
struct buyram_subcommand {
   ...
   buyram_subcommand(CLI::App* actionRoot) {
      ...
      buyram->add_option("amount", amount, localized("The amount of BEOS to pay for RAM, or number of bytes/kibibytes of RAM if --bytes/--kbytes is set"))->required();
...
struct sellram_subcommand {
   ...
   sellram_subcommand(CLI::App* actionRoot) {
      ...
      sellram->add_option("account", receiver_str, localized("The account to receive BEOS for sold RAM"))->required();
...
int main( int argc, char** argv ) {
   ...
   auto getAllProducerJurisdictionForBlock = get->add_subcommand("all_producer_jurisdiction_for_block", localized("Retrieve all jurisdictions for all producer in given block from the blockchain"), false);
   getAllProducerJurisdictionForBlock->add_option("block_number", block_number, localized("The number of block"));
   getAllProducerJurisdictionForBlock->set_callback([&] {

      fc::variant result;
      if( !block_number.empty() )
         result = call(jurisdiction_history_get_all_producer_jurisdiction_for_block, fc::mutable_variant_object( "block_number", std::stol( block_number ) ) );
      else
...
//                                ->check(CLI::ExistingFile);
   ...
   string memo;
   bool pay_ram = false;
   auto transfer = app.add_subcommand("transfer", localized("Transfer BEOS from account to account"), false);
   transfer->add_option("sender", sender, localized("The account sending BEOS"))->required();
   transfer->add_option("recipient", recipient, localized("The account receiving BEOS"))->required();
   transfer->add_option("amount", amount, localized("The amount of BEOS to send"))->required();
