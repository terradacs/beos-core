...
if __name__ == "__main__":
	try:
		...
		producers = node.create_producers(2, "5.0000 PXBTS")
		...
		summary.user_block_status(node, producers[0].name, ResourceResult(_balance="5.0000 PXBTS",_net_weight="458902003.9224 BEOS",_cpu_weight="458902003.9225 BEOS",_ram_bytes=7998080448))
		summary.user_block_status(node, producers[1].name, ResourceResult(_balance="5.0000 PXBTS",_net_weight="458902003.9224 BEOS",_cpu_weight="458902003.9225 BEOS",_ram_bytes=7998080448))
		...
		summary.action_status(node.voteproducer(_voter=producers[1].name,_proxy="",_producers=[producers[0].name]) )
		summary.action_status(node.voteproducer(_voter=producers[1].name,_proxy="",_producers=['beos.tst.abb']), ActionResult(False, "producer is not registered") )
		summary.action_status(node.transfer(_from=producers[1].name,_to=producers[0].name,_quantity="6.0000 PXBTS",_memo="any_memo"), ActionResult(False, "overdrawn balance") )
		summary.action_status(node.transfer(_from=producers[1].name,_to=producers[0].name,_quantity="5.0000 PXBTS",_memo="any_memo") )
		summary.action_status(node.voteproducer(_voter=producers[1].name,_proxy="",_producers=[producers[0].name]) )
