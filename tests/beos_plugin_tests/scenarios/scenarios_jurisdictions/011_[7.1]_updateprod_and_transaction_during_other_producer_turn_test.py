...
if __name__ == "__main__":
	try:
		...
		for jur in jurisdictions_tests:
			...
			result = set_jurisdiction_for_producer(cluster.nodes[0].get_url(), [int(jur[0])])
			log.info(cluster.nodes[0].get_url())
			result = json.loads(result.text)
			try:
				summary.equal(True, result["done"])
			except Exception as x:
				log.info(result)

			log.info("Instantly make transaction on updated jurisdiction")
			resINT, resSTR = cluster.nodes[0].make_cleos_call(["push", "action", "-u", "[{}]".format(jur[0]), "eosio.token",
