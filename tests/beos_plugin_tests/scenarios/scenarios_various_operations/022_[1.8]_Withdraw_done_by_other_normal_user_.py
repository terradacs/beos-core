...
if __name__ == "__main__":
	try:
		...
		summary.user_block_status(node, producers[0].name, ResourceResult(_balance="10.0000 BTS",_net_weight="0.0000 BEOS",_cpu_weight="0.0000 BEOS",_ram_bytes=5448))
		summary.action_status(node.withdraw(_from=accounts[0].name,_bts_to="any_account",_quantity="10.0000 BTS",_memo=""), ActionResult(False, "transaction net usage is too high: 128 > 0") )
		summary.user_block_status(node, accounts[0].name, ResourceResult(_balance="10.0000 BTS",_net_weight="0.0000 BEOS",_cpu_weight="0.0000 BEOS",_ram_bytes=5448))
		
		summary.action_status(node.withdraw(_from=accounts[1].name,_bts_to="any_account",_quantity="10.0000 BTS",_memo="", _authorized_by=accounts[0].name), ActionResult(False, "missing authority of {0}".format(accounts[1].name)) )
		summary.user_block_status(node, accounts[1].name, ResourceResult(_balance="",_net_weight="0.0000 BEOS",_cpu_weight="0.0000 BEOS",_ram_bytes=5448))
		summary.action_status(node.withdraw(_from=accounts[0].name,_bts_to="any_account",_quantity="10.0000 BTS",_memo="", _authorized_by=accounts[1].name), ActionResult(False, "missing authority of {0}".format(accounts[0].name)) )
		summary.user_block_status(node, accounts[0].name, ResourceResult(_balance="10.0000 BTS",_net_weight="0.0000 BEOS",_cpu_weight="0.0000 BEOS",_ram_bytes=5448))
		summary.action_status(node.withdraw(_from=accounts[1].name,_bts_to="any_account",_quantity="10.0000 BTS",_memo=""), ActionResult(False, "overdrawn balance during withdraw") )
		...
		summary.action_status(node.withdraw(_from=accounts[1].name,_bts_to="any_account",_quantity="10.0000 BTS",_memo="", _authorized_by=accounts[0].name), ActionResult(False, "missing authority of {0}".format(accounts[1].name)) )
		summary.user_block_status(node, accounts[1].name, ResourceResult(_balance="",_net_weight="0.0000 BEOS",_cpu_weight="0.0000 BEOS",_ram_bytes=5448))
		summary.action_status(node.withdraw(_from=producers[0].name,_bts_to="any_account",_quantity="10.0000 BTS",_memo="", _authorized_by=accounts[0].name), ActionResult(False, "missing authority of {0}".format(producers[0].name)) )
