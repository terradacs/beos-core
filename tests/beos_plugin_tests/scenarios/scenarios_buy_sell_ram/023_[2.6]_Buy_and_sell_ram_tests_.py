...
if __name__ == "__main__":
	try:
		...
		accounts = node.create_accounts(2, "10.0000 BTS")
		node.run_node()
		
		#Changeparams
		#node.changeparams(["0.0000 BTS"], 10, [10,0,15,5,4000000], [10,0,15,5,2000000], 3000000)
		newparams = {
			"beos" : {
				"starting_block" : 10,
				"next_block" : 0, 
		...
		summary.user_block_status(node, accounts[0].name, ResourceResult(_balance="10.0000 BTS",_net_weight="917804107.8448 BEOS",_cpu_weight="917804107.8450 BEOS",_ram_bytes=15997655448))
		summary.user_block_status(node, accounts[1].name, ResourceResult(_balance="10.0000 BTS",_net_weight="917804107.8448 BEOS",_cpu_weight="917804107.8450 BEOS",_ram_bytes=15997655448))
