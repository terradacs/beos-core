...
if __name__ == "__main__":
	try:
		...
		#Actions
		summary.action_status(node.issue(_from="beos.gateway",_to=accounts[0].name,_quantity="11.0000 PXBTS",_memo="") )
		summary.user_block_status(node, accounts[0].name, ResourceResult(_balance="11.0000 PXBTS",_net_weight="0.0000 BEOS",_cpu_weight="0.0000 BEOS",_ram_bytes=5448))
		...
		summary.action_status(node.issue(_from="beos.gateway",_to=accounts[1].name,_quantity="10.0000 PXBTS",_memo="") )
		node.wait_till_block(35)
		summary.user_block_status(node, accounts[0].name, ResourceResult(_balance="10.0000 PXBTS",_net_weight="917804107.8449 BEOS",_cpu_weight="917804107.8449 BEOS",_ram_bytes=5448))
		node.wait_till_block(35)
		...
		summary.action_status(node.withdraw(_from=accounts[1].name,_bts_to="any_account",_quantity="1.0000 PXBTS",_memo="_memo"), ActionResult(False, "transaction net usage is too high: 128 > 0") )
		node.wait_till_block(85)
		summary.user_block_status(node, accounts[0].name, ResourceResult(_balance="10.0000 PXBTS",_net_weight="1376706161.7673 BEOS",_cpu_weight="1376706161.7674 BEOS",_ram_bytes=15997655448))
		summary.user_block_status(node, accounts[1].name, ResourceResult(_balance="10.0000 PXBTS",_net_weight="458902053.9224 BEOS",_cpu_weight="458902053.9225 BEOS",_ram_bytes=15997655448))
