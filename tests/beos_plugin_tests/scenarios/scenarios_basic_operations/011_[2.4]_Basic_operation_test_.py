...
if __name__ == "__main__":
	try:
		...
		#Changeparams
		#node.changeparams(["0.0000 BTS"], 40, [20,0,35,15,2000000], [20,0,35,15,1000000], 3000000)
		newparams = {
			"beos" : {
				"starting_block" : 20,
				"next_block" : 0, 
		...
		summary.action_status(node.transfer(_from=accounts[0].name,_to=accounts[1].name,_quantity="10.0000 BTS",_memo="any_memo") )
		node.wait_till_block(30)
		summary.action_status(node.transfer(_from=accounts[1].name,_to=accounts[0].name,_quantity="10.0000 BTS",_memo="any_memo"), ActionResult(False, "transaction net usage is too high: 136 > 0") )
		...
		summary.action_status(node.transfer(_from=accounts[1].name,_to=accounts[0].name,_quantity="10.0000 BTS",_memo="any_memo") )
		summary.user_block_status(node, accounts[0].name, ResourceResult(_balance="10.0000 BTS",_net_weight="917804157.8449 BEOS",_cpu_weight="917804157.8449 BEOS",_ram_bytes=15998155448))
