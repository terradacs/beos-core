...
def initialize_beos():
    ...
    try:
        ...
        eosio_actions.push_action("eosio.token", "create", '[ "eosio", "{0} {1}"]'.format(config.CORE_TOTAL_SUPPLY, config.CORE_SYMBOL_NAME), "eosio.token")
        eosio_actions.push_action("eosio.token", "create", '[ "beos.gateway", "{0} {1}"]'.format(config.PROXY_TOTAL_SUPPLY, config.PROXY_ASSET_NAME), "eosio.token")
        ...
        eosio_actions.set_contract("eosio", config.CONTRACTS_DIR + "eosio.bios", "eosio")
        ...
        if config.PRODUCERS_ARRAY:
            ...
            eosio_actions.push_action("eosio", "setprods", '{0}'.format(json.dumps(args)), "eosio")
        ...
        eosio_actions.push_action("beos.init", "storeparams", '[]', "beos.init")
        eosio_actions.push_action("beos.distrib", "storeparams", '[]', "beos.distrib")
