...
class eosio_native_tester : public eosio_interchain_tester
{
  public:
  ...
  void check_user_stats(const account_name& account, const uint64_t expected_balance, const uint64_t expected_ram, const uint64_t expected_net, const uint64_t expected_cpu)
  {
    BOOST_REQUIRE_EQUAL( expected_balance * 1'0000,  get_currency_balance( N(eosio.token), symbol(SY(4, PROXY)) , account ).get_amount() );
    BOOST_REQUIRE_EQUAL( expected_ram, get_staked_balance( account )["ram_bytes"].as_int64() );
    BOOST_REQUIRE_EQUAL( expected_net, get_staked_balance( account )["net_weight"].as_int64() );
    BOOST_REQUIRE_EQUAL( expected_cpu, get_staked_balance( account )["cpu_weight"].as_int64() );
  };
};
...
BOOST_FIXTURE_TEST_CASE( issue_test_01, eosio_native_tester ) try {
  ...
  check_user_stats( N(aaaa), 5, 5448, 0, 0);
  ...
  check_user_stats( N(aaaa), 5, 10666672052, 3671216731379, 3671216731380);

  produce_blocks(4U);
  BOOST_REQUIRE_EQUAL( control->head_block_num(), curr += 4 );
  check_user_stats( N(aaaa), 5, 21333338656, 7342433462758, 7342433462760);

  produce_blocks(5U);
  BOOST_REQUIRE_EQUAL( control->head_block_num(), curr += 5 );
  check_user_stats( N(aaaa), 5, 32000005261, 11013650194137, 11013650194140);
...
BOOST_FIXTURE_TEST_CASE( issue_test_02, eosio_native_tester ) try {
  ...
  check_user_stats( N(aaaa), 10, 10666672052, 3671216731379, 3671216731380);
  produce_blocks(3U);
  BOOST_REQUIRE_EQUAL( control->head_block_num(), curr += 3 );
  issue(N(aaaa), asset::from_string("5.0000 PROXY"));
  produce_blocks(1U);
  BOOST_REQUIRE_EQUAL( control->head_block_num(), curr += 2 );
  check_user_stats( N(aaaa), 15, 21333338656, 7342433462758, 7342433462760);
  produce_blocks(3U);
  BOOST_REQUIRE_EQUAL( control->head_block_num(), curr += 3 );
  issue(N(aaaa), asset::from_string("5.0000 PROXY"));
  produce_blocks(1U);
  BOOST_REQUIRE_EQUAL( control->head_block_num(), curr += 2 );
  check_user_stats( N(aaaa), 20, 32000005261, 11013650194137, 11013650194140);
